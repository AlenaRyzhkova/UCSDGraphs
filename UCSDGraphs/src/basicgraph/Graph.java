...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		// for each vertex count quantity of neighbors and incoming neighbors
		for(int i=0; i<numVertices; i++) {
			List<Integer> neighbors = getNeighbors(i);
			List<Integer> inNeighbors = getInNeighbors(i);
			deg.add(neighbors.size()+inNeighbors.size());
		}
	...
	public static void main (String[] args) {
		GraphLoader.createIntersectionsFile("data/maps/moscow_northwest.map", "data/intersections/moscow_northwest.intersections");
		...
		System.out.println("Moscow north-west district roads / intersections:");
		GraphAdjList moscowGrapth = new GraphAdjList();
		GraphLoader.loadRoadMap("data//maps//moscow_northwest.map", moscowGrapth);
		System.out.println(moscowGrapth);
